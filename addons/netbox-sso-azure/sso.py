...
def azuread_group_permissions(response, user, *args, **kwargs):
    import jwt
    ...
    user_info = jwt.decode(
        jwt_id_token, algorithms=[alg], options={"verify_signature": False}
    )
    ...
    if config.get("AZUREAD_GROUP_PERMISSIONS", None):
        ...
        for group_name, i in groups_dict.items():
            ...
            try:
                ...
                logger.warning(
                    "Azure AD group {} does not exist in NetBox".format(group_name)
                )
            ...
            for ct, permissions in i.items():
                ...
                for permission in permissions:
                    ...
                    logger.info(
                        f"Added permission {permission} to group {group_name} for model {model_name}"
                    )
    else:
        logger.warning(
            "AZUREAD_GROUP_PERMISSIONS not defined in SOCIAL_AUTH_PIPELINE_CONFIG"
        )
...
def azuread_map_groups(response, user, *args, **kwargs):
    import jwt
    from django.contrib.auth.models import Group
    ...
    logger.info(
        "Successfully decoded JWT ID token for user {}".format(user_info["upn"])
    )

    config = SOCIAL_AUTH_PIPELINE_CONFIG

    if (
        "AZUREAD_USER_FLAGS_BY_GROUP" not in config
        and "AZUREAD_GROUP_MAP" not in config
    ):
        raise ImproperlyConfigured(
            "AZUREAD_USER_FLAGS_BY_GROUP and AZUREAD_GROUP_MAP must be defined in SOCIAL_AUTH_PIPELINE_CONFIG"
        )

    flags_by_group = config.get(
        "AZUREAD_USER_FLAGS_BY_GROUP", {"is_superuser": [], "is_staff": []}
    )
